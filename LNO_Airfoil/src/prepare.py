...
if __name__ == "__main__":
    if not os.path.exists(os.path.join(DATA_PATH, "{}_train.npy".format(arg.data_name))) or \
        ...
        if arg.data_name == "Darcy":
            ...
            TRAIN_NUM = 1024
            VAL_NUM = 1024
            x, y1, y2 = load_Darcy(os.path.join(DATA_PATH, "Darcy_{}/piececonst_r{}_N1024_smooth1.mat".format(SRC_RES, SRC_RES)), SRC_RES, OBJ_RES)
            tx, ty1, ty2 = load_Darcy(os.path.join(DATA_PATH, "Darcy_{}/piececonst_r{}_N1024_smooth2.mat".format(SRC_RES, SRC_RES)), SRC_RES, OBJ_RES)
            ...
            TRAIN_NUM = 1000
            VAL_NUM = 200
            Q = np.expand_dims(np.load(os.path.join(DATA_PATH, "airfoil/naca/NACA_Cylinder_Q.npy"))[:,4,:,:], axis=-1)
            X = np.expand_dims(np.load(os.path.join(DATA_PATH, "airfoil/naca/NACA_Cylinder_X.npy")), axis=-1)
            Y = np.expand_dims(np.load(os.path.join(DATA_PATH, "airfoil/naca/NACA_Cylinder_Y.npy")), axis=-1)
            x = []
            ...
            TRAIN_NUM = 1000
            VAL_NUM = 1000
            rr = np.load(os.path.join(DATA_PATH, "elasticity/Meshes/Random_UnitCell_rr_10.npy"))
            sigma = np.load(os.path.join(DATA_PATH, "elasticity/Meshes/Random_UnitCell_sigma_10.npy"))
            theta = np.load(os.path.join(DATA_PATH, "elasticity/Meshes/Random_UnitCell_theta_10.npy"))
            XY = np.load(os.path.join(DATA_PATH, "elasticity/Meshes/Random_UnitCell_XY_10.npy"))
            ...
            matdata = scio.loadmat(os.path.join(DATA_PATH, "plasticity/plas_N987_T20.mat"))
            input = matdata["input"]
            ...
            TRAIN_NUM = 1000
            VAL_NUM = 200
            Q = np.expand_dims(np.load(os.path.join(DATA_PATH, "pipe/Pipe_Q.npy"))[:,0], axis=-1)
            X = np.expand_dims(np.load(os.path.join(DATA_PATH, "pipe/Pipe_X.npy")), axis=-1)
            Y = np.expand_dims(np.load(os.path.join(DATA_PATH, "pipe/Pipe_Y.npy")), axis=-1)
            x = []
