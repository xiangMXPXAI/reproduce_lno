...
if __name__ == "__main__":
    if not os.path.exists(os.path.join(DATA_PATH, "{}_train.npy".format(arg.data_name))) or \
        ...
        if arg.data_name == "Darcy":
            ...
            TRAIN_NUM = 1000
            VAL_NUM = 200
            Q = np.expand_dims(np.load(os.path.join(DATA_PATH, "Pipe_Q.npy"))[:,0], axis=-1)
            X = np.expand_dims(np.load(os.path.join(DATA_PATH, "Pipe_X.npy")), axis=-1)
            Y = np.expand_dims(np.load(os.path.join(DATA_PATH, "Pipe_Y.npy")), axis=-1)
            x = []
