...
if __name__ == "__main__":
    if not os.path.exists(os.path.join(DATA_PATH, "{}_train.npy".format(arg.data_name))) or \
        ...
        if arg.data_name == "Darcy":
            ...
            TRAIN_NUM = 1000
            VAL_NUM = 200
            Q = np.expand_dims(np.load(os.path.join(DATA_PATH, "NACA_Cylinder_Q.npy"))[:,4,:,:], axis=-1)
            X = np.expand_dims(np.load(os.path.join(DATA_PATH, "NACA_Cylinder_X.npy")), axis=-1)
            Y = np.expand_dims(np.load(os.path.join(DATA_PATH, "NACA_Cylinder_Y.npy")), axis=-1)
            x = []
            ...
            XY = np.load(os.path.join(DATA_PATH, "Random_UnitCell_XY_10.npy"))
